...
elseif ($module=='agenda' AND $act=='input'){
  ...
  $acak           = rand(1,99);
  $nama_file_unik = $acak.$name;$mulai=$_POST[thn_mulai].'-'.$_POST[bln_mulai].'-'.$_POST[tgl_mulai];
  ...
  if (!empty($lokasi_file)){
    ...
    mysqli_query($koneksi, "INSERT INTO agenda(tema,
                                  tema_seo, 
                                  isi_agenda,
                                  tempat,
                                  jam,
...
elseif ($module=='agenda' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
  mysqli_query($koneksi, "UPDATE agenda SET tema        = '$_POST[tema]',
                                 tema_seo    = '$tema_seo',
  ...
  header('location:../../media.php?module='.$module);
	...
	$data_gambar = mysqli_query($koneksi, "SELECT gambar FROM agenda WHERE id_agenda='$_POST[id]'");
