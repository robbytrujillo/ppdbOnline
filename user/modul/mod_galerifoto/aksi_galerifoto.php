...
elseif ($module=='galerifoto' AND $act=='input'){
  ...
  $acak           = rand(1,99);
  $nama_file_unik = $acak.$name;$gallery_seo = seo_title($_POST['jdl_gallery']);
  ...
  if (!empty($lokasi_file)){
   ...
   mysqli_query($koneksi, "INSERT INTO gallery(jdl_gallery,
                                    gallery_seo,
                                    id_album,
									username,
                                    keterangan,
...
elseif ($module=='galerifoto' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
     mysqli_query($koneksi, "UPDATE gallery SET jdl_gallery  = '$_POST[jdl_gallery]',
